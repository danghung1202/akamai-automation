...
var self = module.exports = {
    ...
    editTheRuleInPolicy: async (page, ruleName) => {
        ...
        await page.locator(xpthSearchInput)
            .on(puppeteer.LocatorEvent.Action, () => {
                log.white(`Find the rule with name: '${ruleName}'`)
            })
            .fill(ruleName)

        const matchResult = `xpath=//div[contains(@class, "akamaiTableRules")]//table/tbody/tr[td[contains(@class, "name-column")][contains(string(), "${ruleName}")]]/td[last()]/div`
        await page.locator(matchResult)
            .on(puppeteer.LocatorEvent.Action, () => {
                log.white(`Click to cloudlets: '${ruleName}'`)
            }).click()

        self.clickToItemInDropdownOfRule(page, matchResult, "Edit Rule")


    },

    editRedirectUrl: async (page, redirectURL) => {
        await page.locator(`xpath=//form[@name="ruleForm"]//div[@id="rule-settings-redirect"]//input[@name="redirectURL"]`)
            .on(puppeteer.LocatorEvent.Action, () => {
                log.white(`Fill the redirect URL '${redirectURL}'`)
            }).fill(redirectURL)
     ...
     * @param {*} dropdownItemText 
     */
    clickToItemInDropdownOfRule: async (page, rowContainsMenuXpath, dropdownItemText) => {
        const matchItemPath = `${rowContainsMenuXpath}//ul[@class="dropdown-menu"]/li[contains(string(),"${dropdownItemText}")]`
        await page.locator(matchItemPath)
            .on(puppeteer.LocatorEvent.Action, () => {
                log.white(`Click to item ${dropdownItemText} in dropdown`)
            })
